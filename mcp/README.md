...
| Tool | What it does |
|------|--------------|
| `search_exams` | Keyword search across all 1,580 blueprints (vendor, exam code, cert name, topic). Optional `certifying_body` / `vendor_slug` filter. Returns matches with id, domain count, question count, duration, and a practice link. |
...
| `list_certifying_bodies` | All 222 certifying bodies with exam counts (optional `contains` filter) — useful to scope a search. |
