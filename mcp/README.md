...
Cert Atlas publishes machine-readable blueprints for **1,673 exams across 223 certifying bodies** — exam domains and topic weights, passing scores, question counts, durations, prerequisites, retake/renewal rules, languages, official source links, and reviewed study guidance where available. This server exposes that dataset to agents over MCP. **Read-only, public data, no API key.**

## Tools

| Tool | What it does |
|------|--------------|
| `search_exams` | Keyword search across all 1,673 blueprints (vendor, exam code, cert name, topic). Optional `certifying_body` / `vendor_slug` filter. Returns matches with id, domain count, question count, duration, and a practice link. |
| `get_exam_blueprint` | Full blueprint for one exam (by id, code, or name): domains and ranges, logistics, prerequisites, reviewed guidance, aggregate preparation signals, and source verification. |
| `compare_exams` | Compare 2–8 exams side by side: question count, duration, passing score, price, validity, domain count. |
| `list_certifying_bodies` | All 223 certifying bodies with exam counts (optional `contains` filter) — useful to scope a search. |
